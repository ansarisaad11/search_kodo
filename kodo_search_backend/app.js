...
app.get("/userdata", async (req, res) => {
    ...
    if(req.query.orderBy){
      sort.orderBy = req.query.orderBy;
    }
    

    try{
      if(match.search && sort.sortBy){
        console.log('inside both')
        let filterData = data.filter((item) => item.name.toLowerCase().match(match.search.toLowerCase()) || item.description.toLowerCase().match(match.search.toLowerCase()));
        if(sort.sortBy === 'name'){
          res.send(sort.orderBy === 'asc' ? _.sortBy(filterData, sort.sortBy ): _.sortBy(filterData, sort.sortBy).reverse());
        } else {
          res.send(sort.orderBy === 'asc' ? _.sortBy(filterData, (e) => e.dateLastEdited ) : _.sortBy(filterData, (e) => e.dateLastEdited ).reverse());
        }
      }
      else if(match.search){
        console.log('inside search')
        res.send(data.filter((item) => item.name.toLowerCase().match(match.search.toLowerCase()) || item.description.toLowerCase().match(match.search.toLowerCase())));
      } 
      else if (sort.sortBy){
        console.log('inside sort')
        if(sort.sortBy === 'name'){
          res.send(sort.orderBy === 'asc' ? _.sortBy(data, sort.sortBy ): _.sortBy(data, sort.sortBy.reverse()));
        } else {
          res.send(sort.orderBy === 'asc' ? _.sortBy(data, (e) => e.dateLastEdited ) : _.sortBy(data, (e) => e.dateLastEdited ).reverse());
        }
      }
       else {
